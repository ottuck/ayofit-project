...
const FastMethod = ({ navigation }) => {
  ...
  const selectMethod = (seletedValue) => {
    ...
    if (endHour >= 24) {
      ...
      setSelectedDate(
        `${nextDate.getFullYear()}/${currentMonth}/${DateNumber(
          nextDate.getDate()
        )}`
      );
    }

    let endTime = `${nextDate.getFullYear()}/${MonthNumber(
      nextDate.getMonth() + 1
    )}/${DateNumber(nextDate.getDate())} : ${TimeNumber(endHour)}:${TimeNumber(
      endMinute
    )}`;
...
const FastPlan = ({ navigation }) => {
  ...
  if (!selectHour == "") {
    totalDateTime =
      ...
      `${selectMonth}` +
      "/" +
      `${MonthNumber(plusSelectDay)}` +
      ":" +
      `${MonthNumber(totalHourParse)}` +
      ":" +
      `${selectMinute}`;
  } else {
    totalDateTime = null;
    ...
    <LinearGradient colors={["#f7d7be", "#e7a370"]}>
      <PlanContainer>
        ...
        <PlanMethodCView>
          ...
          <Image
            source={require("../images/FastImage/Fast24moon.png")}
            style={styles.Howto}
          />
        </PlanMethodCView>
...
const FastConfirm = ({ navigation }) => {
  ...
  const [year2, month2, day2] = currentDateStr
    .split("/")
    .map((str) => parseInt(str, 10));
  const [hours2, minutes2] = currentTimeStr
    .split(":")
    .map((str) => parseInt(str, 10));

  const [year3, month3, day3] = currentDateEnd
    .split("/")
    .map((str) => parseInt(str, 10));
  const [hours3, minutes3] = currentTimeEnd
    .split(":")
    .map((str) => parseInt(str, 10));
  ...
  // 시작 날짜 (현재 or 셀렉)
  const ConfirmStartTime = formatter
    .format(date4)
    .replace("at", "")
    .replace(",", ", ");
  // 종료 시간 (현재 or 셀렉)
  const ConfirmEndTime = formatter
    .format(date2)
    .replace("at", "")
    .replace(",", ", ");
  // 현재 날짜 (현재 종료)
  const ConfirmTimeEnd = formatter
    .format(date3)
    .replace("at", "")
    .replace(",", ", ");
  return (
    <LinearGradient colors={["#f7d7be", "#e7a370"]}>
      <ConfirmScroll>
        <ConfirmContainer>
          ...
          <ConfirmTView>
            <ConfirmStart>
              <ConfrimSText>Start : {ConfirmStartTime}</ConfrimSText>
            </ConfirmStart>
            <ConfirmEnd>
              <ConfirmEText>
